...
y_preprocessed = transformer.transform(y)
#y_scaler = preprocessing.StandardScaler().fit(y_transformed)
#y_preprocessed = y_scaler.StandardScaler().fit(y_scaler)
...
for i, (train_index, test_index) in enumerate(kf.split(X_preprocessed)):
    ...
    # back transformation of log2
    y_pred_re = np.square(y_pred)
